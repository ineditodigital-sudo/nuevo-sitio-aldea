...
try{
  ...
  cms_pdo()->prepare("INSERT INTO leads (name,email,phone,message,source,url,ip) VALUES (?,?,?,?,?,?,?)")
    ->execute([$name,$email,$phone,$message,$source,$url,($_SERVER['REMOTE_ADDR']??'')]);
}catch(Throwable $e){}
...
$html = '<!doctype html><html><body style="margin:0;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;">'
  ...
  .'<p style="margin:0 0 4px;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#2563eb;font-weight:bold;">Nuevo contacto</p>'
  .'<h1 style="margin:0 0 20px;font-size:22px;color:#0f1c30;">Solicitud desde el sitio web</h1>'
